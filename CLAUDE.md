...
- **The Blocker (Cross-Platform):**
  ...
  - **Shield overlay customisation (iOS):** The `PresenceShieldConfiguration` App Extension (`targets/PresenceShieldConfiguration/ShieldConfigurationExtension.swift`) overrides the default "App Restricted" text with a branded, friendly message: *"Time to connect."* and subtitle instructing the user to open Presence and share a screenshot. Managed by `@bacons/apple-targets` (type `"shield-config"`). The primary button label is "Open Presence" — the companion `PresenceShieldAction` extension (below) handles the tap by dismissing the shield so the user is dropped on the home screen and can manually launch Presence.
  - **Shield action handler (iOS):** The `PresenceShieldAction` App Extension (`targets/PresenceShieldAction/ShieldActionExtension.swift`) is the `ShieldActionDelegate` that responds to the shield's buttons. The handler is intentionally minimal: `primaryButtonPressed → .close` (dismisses the shield and the blocked app, returning the user to the home screen), `secondaryButtonPressed → .defer`, `@unknown default → .none`. No app-launch logic, no App Group reads, no localised strings — Apple's default close behaviour is good enough and we no longer try to deep-link into Presence from the extension. Managed by `@bacons/apple-targets` (type `"shield-action"`). Principal class name MUST be `ShieldActionExtension` (the plugin hard-codes `$(PRODUCT_MODULE_NAME).ShieldActionExtension` into the extension's Info.plist). **UX note:** the Configuration extension's button label still says "Open Presence" but the action just closes the shield — the user has to find the Presence icon themselves after the blocked app dismisses. If you ever want the button to actually launch the host app, the previously-used pattern was `NSClassFromString("UIApplication")` + `perform(NSSelectorFromString("openURL:"), with: URL(string: "presence://shield"))` to bypass the compile-time `UIApplication.shared.open` unavailability in extensions. We removed it because the runtime-selector lookup is fragile under App Review scrutiny and the default `.close` path is acceptable.
    - **App Group entitlement:** the extension still declares `group.com.franciccio.presence` in `targets/PresenceShieldAction/expo-target.config.js` even though the current Swift code doesn't read it. Keep it set so future additions (e.g. logging a tap timestamp the main app can consume) don't need separate provisioning churn. Same caveat as gotcha #18: the App Group must be enabled for the extension's App ID (`com.franciccio.presence.PresenceShieldAction`) in the Apple Developer portal, and cached EAS provisioning profiles deleted (`eas credentials --platform ios`) so fresh profiles include both this extension and the existing ones.
    - **App Group entitlement (CRITICAL):** because it now reads the App Group, `targets/PresenceShieldConfiguration/expo-target.config.js` MUST declare `"com.apple.security.application-groups": ["group.com.franciccio.presence"]`. Without it, `UserDefaults(suiteName:)` returns nil in the extension and the shield silently falls back to English (this is why it stayed English even after a rebuild). The App Group must also be enabled for the extension's App ID (`com.franciccio.presence.PresenceShieldConfiguration`) in the Apple Developer portal, and cached EAS provisioning profiles deleted (`eas credentials --platform ios`) so fresh profiles include it (same as gotcha #18 for `PresenceMonitor`).
  - **Screen Time auth re-request (CRITICAL — `denied` cannot be re-prompted):** Auth is re-checked on every foreground / cold start in `checkAndUpdateShield()`, and on save (`block-time.tsx`'s save calls `checkAndUpdateShield()`; onboarding `step-4-goal.tsx` calls `ensureScreenTimeAuth()`). **`checkAndUpdateShield()` deliberately does NOT gate on `getAuthorizationStatus()`** — that property is a *cached* value that is stale on a warm resume (it does not update when the user changes Screen Time in Settings while the app is only suspended, and the Combine publisher does not re-emit on resume). Instead it calls `requestAuthorization()` **directly**: a LIVE system check that presents the native modal when the real state is `notDetermined`, and throws silently (no UI) when already approved or denied. The catch then reads the (now-refreshed) status to detect `denied`. The two revoked states differ:
    - **`notDetermined`** — fresh install, the user turned **Screen Time off globally** (Settings → Screen Time → Turn Off), OR the user toggled Screen Time off **for the Presence app specifically** (Settings → Presence → Screen Time off). In all of these `requestAuthorization()` **can** re-show the native FamilyControls system modal, so calling it directly re-prompts.
    - **`denied`** — only reached if the user taps **"Don't Allow"** on the system modal. iOS then **permanently forbids** `requestAuthorization()` from re-showing the modal (same privacy rule as Camera / Photos / Location). The only recovery is `alertScreenTimeRevoked()` — a native `Alert.alert()` with "Open Settings" (deep-links to `app-settings:`) and "Not now", throttled once per app session (`_screenTimeAlertShown`; `force=true` from `ensureScreenTimeAuth`).
    - **Event-driven detection:** `native-src/PresenceScreenTime.swift` is an `RCTEventEmitter` that subscribes to `AuthorizationCenter.shared.$authorizationStatus` (Combine) in `startObserving()` with `.dropFirst().removeDuplicates()`. **`removeDuplicates()` is essential**: `requestAuthorization()` re-assigns the published value (even to the same status), so without it the emit → `checkAndUpdateShield()` → `requestAuthorization()` → emit cycle loops forever. The emitted `onScreenTimeAuthChanged` event is handled in `startShieldEngine()` via `addScreenTimeAuthChangedListener()`, which simply re-runs `checkAndUpdateShield()` (it performs the live re-prompt itself).
    - **Cold-start hydration gate:** `routineStore` persists via AsyncStorage but has no hydration flag, so `startShieldEngine()` waits for `useRoutineStore.persist.hasHydrated()` / `onFinishHydration()` before the first `checkAndUpdateShield()`. Without this, a cold start (which is what happens when the app is killed) reads `blockTimeUtc=null`, bails at the `routineReady` guard, and never reaches the auth check or shield re-evaluation.
  - **Block time scheduling — 2-minute minimum window:** When the user saves a block time in `block-time.tsx` or `step-4-goal.tsx`, a toast immediately informs them whether the shield starts today or tomorrow. If the chosen time is less than 2 minutes in the future (or already past for today), the toast warns: *"Less than 2 minutes away — your shield will start tomorrow at [time]"*. Otherwise it confirms today's start. The schedule is still saved — the native `DeviceActivitySchedule` repeats daily so it correctly fires tomorrow.
...
- **Validation Rules (Phase 9 — challenge system).** Three strict gates. Screenshot age runs upstream in `handleUploadProof` (iOS-only PHAsset check); the other two live in `lib/ocr.ts → validateOCRText(text, contacts: Contact[], challenge: ActiveChallenge | null)`. Every block cycle assigns exactly one **challenge** = `(contact, theme, word)` and both the specific contact name **and** the specific word must appear in the OCR text to pass. See §6F for the full challenge architecture; the rules below cover the OCR side only.
  1. _Screenshot age (STRICT — iOS only, runs BEFORE OCR):_ Same as before — `MediaLibrary.getAssetInfoAsync(asset.assetId).creationTime` must be within `MAX_SCREENSHOT_AGE_HOURS` (currently `6`). Falls open on Android + on any metadata error. Fails with `failureTooOldTitle/Body`.
  2. _Effort (STRICT):_ Text block > 4 words. Catches blank / OCR-failed uploads. Fails with `low_effort`.
  3. _Contact name (STRICT):_ **When a challenge is active,** the challenge's specific contact name must appear in the OCR text (case-insensitive substring). Fails with `no_contact_name` and the toast names the specific contact via `failureNoContactNamedBody`. **Fallback path (no challenge)** — any trusted contact's name is accepted. This fallback exists for the pre-challenge legacy state and the "user has no contacts yet" first-run edge case; the challenge is the norm.
  4. _Challenge word (STRICT — Phase 9 anti-cheat gate):_ **Only enforced when a challenge is active.** The `challenge.word` must appear as a whole-word substring in the OCR text (case-insensitive). Fails with `no_challenge_word`; the toast names the required word via `failureNoWordBody`. This is what makes screenshot-recycling useless: the word rotates every block cycle (server picks it from the theme's keyword pool, avoiding the user's last 2 words for that contact).
  - **Removed / never re-add:** the old English-only "messaging signals" (send/delivered/etc.), the multi-pattern recency check (today/yesterday/weekdays/12h clock), and the soft "thematic keyword scoring" pass. All three were replaced by the deterministic challenge-word gate. If you're tempted to "improve" verification by adding fuzzy keyword scoring back, don't — the challenge word IS the keyword gate now, and it's strict on purpose.
  - The validator returns `{ valid, reason?, matchedContactId?, matchedContactName?, matchedThemeId?, matchedChallengeWord?, requiredChallengeWord?, requiredContactName? }`. The `reason` union is `"no_text" | "low_effort" | "no_contact_name" | "no_challenge_word" | "ocr_error" | "unavailable"`. On success, `matchedChallengeWord` and `matchedThemeId` flow through `onConnectionVerified(wasBypass, { contactId, themeId, challengeWord })` into `connection_proofs` (drives analytics word-bank + rotation avoidance) and `markThemeUsedRemote` (drives theme rotation). On `no_challenge_word` failure the toast reads `failureNoWordBody` interpolated with the required word, so the user knows exactly what to include.
  - **Why creationTime works:** the iOS Photos library stamps every screenshot with `PHAsset.creationDate` at capture time. Users can't fake this without altering the Photos database (jailbreak territory). A screenshot copied from someone else, or saved 3 days ago and uploaded today, all have stale `creationDate` values. The only way to defeat the gate is to take a fresh screenshot today — which is exactly the behaviour we want. **Limitation:** a user could take a fresh screenshot today of an old conversation (creationTime is now, content is old). The strict name gate combined with the loose thematic match catches enough of this in practice; we don't try to OCR-detect "old content in a fresh screenshot."
  - **The Relief Valve (CRITICAL UX):** If the OCR fails to verify a screenshot _twice in a row_, the UI must present a "Manual Bypass" button. If the user clicks this, allow them through the Shield (unblock apps) but log the bypass in Supabase. Do not trap a paying user out of their phone due to a vision glitch.
    -- **Result:** Success triggers haptics + unblocks Shield. Failure shows an elegant toast — generic "We couldn't verify this connection..." for `low_effort` / `no_text` / `ocr_error`, the tailored "We couldn't read a trusted contact's name..." for `no_contact_name`, and "We need a fresher screenshot. This one was taken a while ago..." for the upstream age gate.
  ...
  - **`expo-media-library` permission (gotcha):** The screenshot-age check uses `MediaLibrary.getAssetInfoAsync`. **No new permission prompt** — the package shares the `NSPhotoLibraryUsageDescription` Info.plist key with `expo-image-picker`, which the user already grants during onboarding step 8 (Photo Library is in the REQUIRED bucket). We do **not** call `MediaLibrary.requestPermissionsAsync()` ourselves; we rely on the picker's existing grant and let `getAssetInfoAsync` fail silently if for some reason it can't read (try/catch around it). Adding the package required an EAS rebuild — it's a native module.
...
- `contact_themes` — id, contact_id, user_id, theme_text, keywords[], used_at, created_at. `used_at IS NULL` = "still in rotation". `regenerateThemes` deletes only the `used_at IS NULL` rows and inserts a fresh batch, preserving history for analytics. **Phase 9:** `keywords[]` is now the **challenge-word pool** — variable count (3–12 per theme, whatever the prompt truly warrants), tight and texteable. See §6F + the `generate-themes` bullet in §7 for the new keyword contract.
- `connection_proofs` — gained `contact_id`, `theme_id`, and (Phase 9) `challenge_word text`. Populated by `syncPendingConnections`; `challenge_word` records the exact keyword that unblocked the shield so analytics can compute distinct-words-matched and `lib/blockChallenge.ts` can avoid repeating recent words.
- `block_challenges` (**Phase 9, new**) — id, user_id, contact_id, theme_id (nullable), challenge_word, assigned_at, resolved_at, created_at. Single source of truth for the current cycle's challenge. Partial unique index `block_challenges_active_uniq` enforces one active (unresolved) row per user. Written by `assignChallengeIfNeeded`, marked resolved by `resolveActiveChallenge`.
- `profiles` — Phase 9 added `achievements_earned integer[] not null default '{}'` mirroring the local `useUserStore.achievementsEarned`. Written by `syncPendingConnections` alongside `lifetime_connections` / `current_streak`.
...
**Analytics (`app/(tabs)/analytics.tsx`, Phase 9):**
- Top stats row: connections | streak | time reclaimed (kept).
- **Achievements strip** — pill row of `ACHIEVEMENT_MILESTONES` (`[3, 7, 14, 30, 60, 100, 250]`) with visual state: unlocked (filled brown/tan), next (outlined tan, highlighted), locked (muted). Reads `useUserStore.achievementsEarned`. Copy under the strip shows either `analytics.achievementNext` (next threshold) or `analytics.achievementAllDone`.
- **Word bank card** — `matched / total` distinct challenge keywords the user has ever landed. Progress bar. `matched` is the distinct set of `pendingConnections.challengeWord` values; `total` is the distinct set of `keywords[]` across every theme configured. Hidden when the user has no contacts.
- **Prompts explored card** — `used / total` themes touched (`themes.filter(t => t.usedAt !== null).length` summed across contacts vs total theme count). Progress bar. Same visibility rule.
- **"Your circle" section** — per-contact cards sorted oldest-`lastConnectionAt` first. Each card shows last-connection-ago (stale ≥ 7 days renders in tan bold), prompts-touched progress bar, per-contact word-landed progress bar, and a "Suggestion" callout with one composed unused-theme line (same string the warm-up notification would use).
- "Add a trusted contact in your profile..." empty state when `contacts.length === 0`.
- **App Rating Strategy (CRITICAL):** Use `expo-store-review` to trigger the native App Store rating prompt. You must ONLY trigger this immediately after a successful OCR verification (while the user is in a high-dopamine state), and ONLY on the user's exactly 3rd lifetime successful connection. Do not spam them early on. This is separate from — and coexists with — the Phase 9 achievement notifications (which fire on every milestone).

### F. Challenge System (Phase 9 — the gamified anti-cheat gate)

Replaces the fuzzy soft-keyword scoring with a **deterministic per-cycle challenge**: `(contact, theme, word)`. Every block cycle the app picks one word from the contact's rotation-selected theme and demands the user include that exact word in their real text message. That word — plus the specific contact's name — are the two hard OCR gates. The word rotates every cycle, so re-uploading an old screenshot no longer works.

**Data model** — see §6E for the schema. Key rows:
- `block_challenges` — one active row per user (partial unique index enforces it).
- `connection_proofs.challenge_word` — records which word satisfied each verification.
- `profiles.achievements_earned` — mirrors the local milestone-earned set.

**Local stores:**
- `useShieldStore.activeChallenge: ActiveChallenge | null` — mirrors the DB active row. Shape: `{ id, contactId, contactName, themeId, themeText, word, assignedAt, themesStale }`. `themesStale: true` means the pick fell back to a previously-used word because the chosen theme had zero keywords — Home + Contact Editor surface a "regenerate prompts / edit answers" nudge in that case.
- `useShieldStore.pendingConnections[i].challengeWord: string | null` — every synced proof also carries this into Supabase.
- `useUserStore.achievementsEarned: number[]` — milestones already celebrated. `lastAckAchievement: number` — highest milestone whose Home banner has been dismissed. `addAchievement(m)`, `acknowledgeAchievement(m)`, `setAchievements(list)`.

**Source of truth (CRITICAL):** `block_challenges` on the server is the source of truth for "what's the current challenge?" — NOT the local Zustand `activeChallenge`. Local persist is a warm cache that gets reconciled from the server on cold start via `hydrateActiveChallengeFromServer()` and on every write-path via `scheduleWarmupNotification` (which calls hydrate before assign). If you ever add a new trigger that mutates `activeChallenge`, either route it through the server first or make sure hydrate runs before you next read.

**Assignment (`lib/blockChallenge.ts → assignChallengeIfNeeded({ force? })`):**
1. Pick the contact via `pickNextContact` (existing rotation — oldest-last-connection first).
2. Pick the theme via `pickNextTheme` (unused first, then oldest-used).
3. Pick a word from `theme.keywords`, **avoiding the last `AVOID_LAST_N_WORDS = 2`** words used for that contact (from `pendingConnections.challengeWord` history, newest-first, synced or not).
4. **Stale-themes fallback:** if the chosen theme has zero keywords (an older AI output), re-use a previously-used word for that contact + set `themesStale: true`.
5. **Zero-history fallback:** if no prior words exist either, silently `await regenerateThemes(contactId)`, re-pick from the fresh output.
6. Insert into `block_challenges` (best-effort; `resolveAllActiveOnServer` first clears any straggler so the partial unique index doesn't reject).
7. Idempotent by default: no-op if `activeChallenge` already set. **`force: true` is reserved for one path**: when you've *just* resolved a challenge and know you need a new one (only `resolveActiveChallenge` → `scheduleWarmupNotification` fits this, and even there the resolve already cleared `activeChallenge` so `force` is redundant). **DO NOT** pass `force: true` in the app-start / language-change / schedule-save paths — it re-rolls the word on every relaunch, which users perceive as a bug ("why did my challenge word change when I closed and opened the app?"). If you're tempted to force, ask first whether hydrating from server would give you the correct behaviour.

**Hydration (`hydrateActiveChallengeFromServer()`):** reads the current unresolved `block_challenges` row (with `contact` and `theme` FK joins so the local contacts store doesn't need to be hydrated first) and mirrors it into `useShieldStore.activeChallenge`. If the server has no active row, clears local. On any network / RLS error, keeps local state as-is. Called by:
- `startShieldEngine` cold-start sequence (see `lib/shieldEngine.ts`), immediately after `syncPendingConnections` so the sync's `block_challenges.resolved_at` catch-up runs *before* we read.
- `scheduleWarmupNotification` (every re-bake trigger), immediately before `assignChallengeIfNeeded` so a fresh-install with server state never gets overwritten.

**Resolution (`resolveActiveChallenge()`):** called from `onConnectionVerified`. Returns `{ contactId, themeId, word }` from the snapshot, clears the local `activeChallenge`, marks the server row `resolved_at = now()`. The word flows into the proof row via `matched.challengeWord`. **Offline-resolve catch-up:** the fire-and-forget server update inside `resolveActiveChallenge` will fail silently if the device is offline. `syncPendingConnections` heals this — for every proof it inserts with a `challenge_word`, it also `UPDATE block_challenges SET resolved_at = <ts> WHERE user_id/contact_id/challenge_word matches AND resolved_at IS NULL`. Without this, cold-start hydration would re-load the still-active-on-server row after an offline verify.

**Assignment / hydration triggers (must stay in sync):**
1. **Cold start** — `startShieldEngine` runs `syncPendingConnections` → `hydrateActiveChallengeFromServer` → `checkAndUpdateShield` in that order after routine store hydration finishes.
2. `scheduleWarmupNotification` — `hydrateActiveChallengeFromServer()` + `assignChallengeIfNeeded()` (no force). Fires at: app start, schedule save, `onConnectionVerified`, `regenerateThemes`, language change.
3. On-demand safety net in `app/(tabs)/index.tsx` — a `useEffect` that assigns when `isBlocked && !activeChallenge && contacts.length > 0`. Covers extension-fired blocks that never routed through `scheduleWarmupNotification`.
4. Post-verify / manual bypass — `resolveActiveChallenge` clears local + server, then `onConnectionVerified → scheduleWarmupNotification` hydrates (finds nothing) and assigns fresh.

**Achievements (`ACHIEVEMENT_MILESTONES = [3, 7, 14, 30, 60, 100, 250]`):**
- `newlyEarnedMilestones(lifetime, alreadyEarned)` → list of thresholds crossed on this connection.
- On every verify, `onConnectionVerified` runs this against the freshly-incremented lifetime count. Each new milestone is added via `useUserStore.addAchievement` + fires a one-off local push via `fireAchievementNotification(milestone)` (no identifier — stacks in tray).
- Home banner surfaces the highest earned-but-unacknowledged milestone; tapping "Nice" calls `acknowledgeAchievement(m)`. Persists via Zustand so the banner doesn't re-appear after relaunch.
- Analytics shows the whole strip with unlocked / next / locked visual states.
- Achievements mirror to `profiles.achievements_earned` in `syncPendingConnections` alongside the lifetime/streak update.

**Notifications:**
- **Warm-up body** priority order: (1) `warmupBodyChallenge` when `activeChallenge` is set — "Shield up in 15 min. Text {name} something with the word \"{word}\" to unlock." (2) `warmupBodyTargeted` (legacy — no active challenge but a theme exists), (3) `warmupBodyNoTheme`, (4) `warmupBody` generic. `scheduleWarmupNotification` calls `hydrateActiveChallengeFromServer()` then `assignChallengeIfNeeded()` (no force) via a **lazy `require`** so the `notifications → blockChallenge → contactsSync → notifications` import cycle is broken.
- **Achievement notification** — `fireAchievementNotification(milestone)` uses `analytics.achievementTitle` / `achievementBody` with `{{count}}`, no identifier so multiple milestones in a single verify each land as their own tray entry. Data: `{ type: "achievement", milestone }`.

**OCR — see §6D** for the gate order and toast copy. The validator signature is now `validateOCRText(text, contacts, challenge)`; `runOCRValidation` takes the same three args. On success, `matchedChallengeWord` flows into the proof row.

**Home blocked-state UI (`app/(tabs)/index.tsx`):**
- Achievement banner (tan, dismissible) — shows when `achievementsEarned` has any value > `lastAckAchievement`.
- Challenge card — contact name (serif display), required-word chip (dark-brown / tan filled), prompt-idea preview (italic, `themeText` with `{name}` swapped). Only rendered when `isBlocked && activeChallenge`.
- Stale-prompts nudge — rendered when `activeChallenge.themesStale`. CTA routes to `/contacts` for regen / answer edit.

**Contact editor (`components/ui/ContactEditorSheet.tsx`):**
- Stale banner appears above the themes list when `themes.some(t => t.keywords.length === 0)`.

**`generate-themes` edge function keyword contract (Phase 9 rewrite):**
- Keywords are **challenge words** — they will be shown verbatim to the user ("This time, text Marie something with the word RESPECT") and demanded as a strict OCR match. The prompt now explicitly frames them that way.
- **Variable count**: 3–12 per theme, whatever the specific prompt warrants. A narrow prompt ("congratulate on promotion") gets ~4 words ("congrats", "promotion", "promoted", "raise"); a broad prompt ("check in on family") gets more.
- **Texteable, casual, colloquial** — words a friend would naturally slip into a real text. NOT formal or abstract vocabulary. `MIN_KEYWORDS = 3`, `MAX_KEYWORDS = 12` cap.
- **Server-side filler blocklist** — new `GENERIC_FILLER` set drops greetings / auxiliary verbs / generic emotion words in EN/ES/FR/IT/PT (would collide with unrelated prompts). Existing `BANNED` media word blocklist stays.
- **Answer-language rule preserved** — still detects the primary language of Q1–Q4 answers and writes all keywords in that language, since the user will text in the language they wrote the answers in and the challenge-word match must literal-string succeed.
...
- **Edge Functions:** Five functions deployed via `supabase functions deploy <name>`:
  ...
  - `generate-themes` — writes 6 personalised prompts per contact via OpenAI (`gpt-4o-mini`, JSON mode). Request body: `{ contact_id, language? }`. Verifies the caller JWT, reads the contact row, calls OpenAI with a strict system prompt, deletes the contact's existing `used_at IS NULL` themes, inserts the new batch, returns them. Requires `OPENAI_API_KEY` secret (`supabase secrets set OPENAI_API_KEY=sk-...`).
    - **Language priority (CRITICAL — easy to get wrong):** the system prompt is a 3-step instruction: (1) detect the primary language of the Q1-Q4 answers; (2) write *all* themes and *all* keywords in that detected answer-language; (3) only if the answers are blank/nonsense, fall back to the client-passed app language. Don't revert it.
    - **Output format (Phase 9 — challenge-word contract):** themes are short action phrases in lowercase infinitive form with a literal `{name}` placeholder. Keywords are the **challenge-word pool** — 3–12 texteable, casual words per theme (`MIN_KEYWORDS = 3`, `MAX_KEYWORDS = 12`), variable count, whatever the prompt warrants. The prompt explicitly frames these as words the user will see and be told to include in their real message ("This time, text Marie something with the word RESPECT"). Bans formal ("congratulate", "achievement", "endeavor"), generic ("happy", "good", "hope"), and media words ("meme", "gif"). Do not revert to a fixed keyword count — the natural pool size is prompt-specific and stretching it produces low-signal challenge words that fail unfairly.
    - **Server-side sanitiser:** strips banned media words (`meme`, `gif`, `sticker`, etc.) AND generic filler / greeting words in EN/ES/FR/IT/PT (`GENERIC_FILLER` set) from keyword arrays; drops themes whose text contains a banned word; enforces `MIN_KEYWORDS` (3) after filtering so a theme that only produced filler is dropped rather than assigned an unfair challenge.
...
- **Expo Notifications (local only):** Three notification types implemented in `lib/notifications.ts`. Bodies are baked in at schedule time (except achievement, which fires immediately):
  1. **Warm-up:** Fires 15 min before block time. **Frequency-aware**: `daily` → one repeating `DAILY` trigger; `5x` → repeating `WEEKLY` triggers Mon–Fri; `weekends` → `WEEKLY` Sat/Sun (each weekday has its own `presence-warmup-<n>` id; the 15-min subtraction can shift the fire weekday across midnight). `cancelWarmupNotification()` clears the daily id + all 7 weekday ids.
     - **Dynamic per-contact body (Phase 9):** `buildWarmupBody()` reads `useShieldStore.activeChallenge` first — if set, returns `warmupBodyChallenge` with `{{name}}` and `{{word}}` interpolated ("Shield up in 15 min. Text {name} something with the word \"{word}\" to unlock."). Falls back to `warmupBodyTargeted` (composed theme line), `warmupBodyNoTheme` (name only), or `warmupBody` (no contacts). The challenge is assigned/refreshed inline in `scheduleWarmupNotification` via a **lazy `require("@/lib/blockChallenge")`** call so the notifications ↔ blockChallenge ↔ contactsSync import cycle stays broken.
     - **Re-bake triggers (FIVE — must keep all in sync):** body is static once scheduled, so it must be re-baked on every state change that affects rotation or language. (1) app start via `initNotifications()`; (2) schedule save in `block-time.tsx`; (3) `onConnectionVerified()` (the just-connected contact drops to the bottom of the rotation, so the next fire should target someone fresher); (4) `regenerateThemes()` success (new theme text); (5) `i18n.changeLanguage` event in `_layout.tsx` (the same listener that mirrors language to the shield extension — otherwise a user who scheduled in Italian and then switched to English would receive hybrid-language notifications with Italian template strings around an English theme line). Each rebake also refreshes the block challenge via `assignChallengeIfNeeded({ force: true })`, so the notification body and Home always name the same `(contact, word)`.
  3. **Achievement (Phase 9 — one-off):** `fireAchievementNotification(milestone)` fires immediately when a lifetime-count milestone is crossed inside `onConnectionVerified`. No identifier so multiple milestones stack. Title/body use `notifications.achievementTitle` / `achievementBody` interpolated with `{{count}}`. Data payload: `{ type: "achievement", milestone }`.
...
5. **Navigation Interception (Anti-Cheat & Hard Stops):**
   ...
   - **Routine-edit lock while blocked (CRITICAL anti-cheat):** While `useShieldStore.isBlocked` is true, the user must NOT be able to lift the shield by reconfiguring instead of verifying. `app/block-time.tsx` (Schedule + "Remove schedule") and `app/blocked-apps.tsx` (app selection) render `components/ui/LockedWhileBlocked.tsx` instead of their editable UI — a locked notice whose CTA routes to the home tab to upload proof. The only ways out of an active block are a verified connection or the 2-fail manual bypass. (Trusted-contacts editing is NOT locked — changing it can't lift the shield.)
...
20. **`ShieldConfigurationExtension.swift` — Imports & Override Signatures:**
   - `targets/PresenceShieldConfiguration/ShieldConfigurationExtension.swift` must `import UIKit` (for `UIColor`, used in the branded `ShieldConfiguration` colors) and `import ManagedSettingsUI` + `import ManagedSettings`. Missing `UIKit` → "cannot find 'UIColor' in scope".
...
- [ ] **Phase 9 challenge-system migration applied:** Run the second migration block in `supabase/schema.sql` (truncates `contact_themes` + `connection_proofs` + `block_challenges`; adds `profiles.achievements_earned` and `connection_proofs.challenge_word`; creates `block_challenges` table with its RLS policy + partial unique index). **Destructive: wipes all themes + proofs.** Fine pre-launch (no users) — must NOT run after production launch. If users exist by then, replace `truncate` with an `alter table ... alter column keywords set default '{}'` migration and force a client-side regen next time each user opens their contact editor.
- [ ] **Redeploy `generate-themes`:** `supabase functions deploy generate-themes`. The Phase 9 prompt rewrite changes keyword count + banned-word semantics; old keyword arrays (5–8 fixed, formal words allowed) will produce unfair challenge words if left in place.
...
- [ ] **PresenceShieldAction App ID + entitlements:** Separate extension target. In Apple Developer Portal create an App ID for `com.franciccio.presence.PresenceShieldAction`, enable the **App Groups** capability and add `group.com.franciccio.presence`. Then run `eas credentials --platform ios` to delete cached provisioning profiles so EAS generates fresh ones covering the new extension (otherwise the EAS build fails with a "no matching profile" error, or builds without the extension — in which case the shield's primary button falls back to iOS's default behaviour instead of our `.close` response).
...
- [ ] **`expo-media-library` native module compiled in:** The screenshot-age strict gate (§6D rule 1) uses `MediaLibrary.getAssetInfoAsync` from `expo-media-library@~18.x`. This is a native module, so a JS-only reload against an older dev client throws "Cannot find native module 'ExpoMediaLibrary'". Verify the dev client and the production build were both compiled with this dep in `package.json`.
...
- **[ ✅ DONE ] Phase 9: Gamified Challenge System** — Replaced the fuzzy soft-thematic gate with a deterministic per-cycle challenge: every block cycle picks a `(contact, theme, word)` from the contact's texteable keyword pool. That word + the specific contact's name become the two hard OCR gates — re-uploading an old screenshot no longer bypasses. Wired: `block_challenges` table + partial unique index, `connection_proofs.challenge_word`, `profiles.achievements_earned`, `lib/blockChallenge.ts` (assign / resolve / stale-themes fallback / avoid-last-2-words rotation), new `no_challenge_word` OCR reason with tailored toast, warm-up notification body variant naming the required word, achievement milestones `[3, 7, 14, 30, 60, 100, 250]` with local push + Home banner + Analytics strip, word-bank + prompts-explored progress cards, contact-editor stale banner, `generate-themes` prompt rewritten for variable-count texteable keyword pools (3–12, natural / casual only). See §6F for the full architecture.
