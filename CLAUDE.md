...
- **The Blocker (Cross-Platform):**
  ...
  - **Shield overlay customisation (iOS):** The `PresenceShieldConfiguration` App Extension (`targets/PresenceShieldConfiguration/ShieldConfigurationExtension.swift`) overrides the default "App Restricted" text with a branded, friendly message: *"Time to connect."* and subtitle instructing the user to open Presence and share a screenshot. Managed by `@bacons/apple-targets` (type `"shield-config"`). The primary button label is "Open Presence" — the companion `PresenceShieldAction` extension (below) handles the tap and actually opens the app.
  - **Shield action handler (iOS):** The `PresenceShieldAction` App Extension (`targets/PresenceShieldAction/ShieldActionExtension.swift`) is the `ShieldActionDelegate` that responds to the shield's primary button. Without it, tapping "Open Presence" just dismisses the shielded app and drops the user on the home screen — they then have to find the Presence icon themselves. With it, the tap launches Presence via the `presence://shield` URL scheme (the scheme `presence` is registered in `app.json`). Managed by `@bacons/apple-targets` (type `"shield-action"`). Principal class name MUST be `ShieldActionExtension` (the plugin hard-codes `$(PRODUCT_MODULE_NAME).ShieldActionExtension` into the extension's Info.plist).
    - **Opening the host app — runtime-selector workaround (CRITICAL, don't "fix"):** `UIApplication.shared.open(_:)` is marked unavailable in app extensions at compile time. The extension uses `NSClassFromString("UIApplication")` + `value(forKey: "sharedApplication")` + `perform(NSSelectorFromString("openURL:"), with: url)` to bypass the compile-time check. This is the well-known pattern used by shipping Screen Time apps (Opal, Brick, etc.) and runs at runtime against UIKit which is loaded in shield-action extensions. **Do not "modernise" this to `UIApplication.shared.open` or to `extensionContext?.open(_:)`** — the first is a compile error in extensions, and `ShieldActionDelegate` doesn't expose an `extensionContext` property, so the second silently fails. App Review has accepted this pattern in production Screen Time apps but it does use runtime class-string lookup, so be ready to defend it if a reviewer asks.
    - **App Group entitlement:** mirrored from the Configuration extension via `targets/PresenceShieldAction/expo-target.config.js`. We don't read App Group data here yet, but keeping the entitlement set means later additions (e.g. localised confirmation toasts) won't need separate provisioning churn. Same caveat as gotcha #18: the App Group must also be enabled for the extension's App ID (`com.franciccio.presence.PresenceShieldAction`) in the Apple Developer portal, and cached EAS provisioning profiles deleted (`eas credentials --platform ios`) so fresh profiles include both this extension and the existing ones.
    - **Localisation:** the extension can't use react-i18next, so its copy is a Swift `localizedStrings(for:)` table (en/es/fr/it/pt). It reads the language code from the App Group key `appLanguage`, which the main app mirrors via `ScreenTimeModule.setAppLanguage(code)` — called from `_layout.tsx` on mount and on every i18n `languageChanged` event. To add/adjust shield copy you must edit the Swift table (and rebuild), not the JSON locale files.
...
- **Validation Rules.** Three strict gates + two soft signals. Two of the three strict gates live inside `lib/ocr.ts → validateOCRText(text, contacts: Contact[])`; the third (screenshot age) lives upstream in the home tab's `handleUploadProof` because it reads the iOS Photos library asset metadata, not the OCR text.
  1. _Screenshot age (STRICT — iOS only, runs BEFORE OCR):_ Implemented in `app/(tabs)/index.tsx → handleUploadProof` via `expo-media-library.getAssetInfoAsync(asset.assetId).creationTime`. Rejects any upload where the PHAsset's `creationDate` is more than `MAX_SCREENSHOT_AGE_HOURS` (currently `6`) old. Fails with `failureTooOldTitle/Body` toast, increments `ocrFailCount` (so the 2-fail Manual Bypass still kicks in). **Falls open on Android and on any metadata error:** the check is wrapped in try/catch and skips silently if `assetId` is missing, `getAssetInfoAsync` throws, or `creationTime` is not a number — so a metadata API hiccup never blocks a legitimate upload. The fall-open path also covers cross-platform: Android `PHAsset.assetId` is iOS-only, so on Android this gate is effectively absent. Re-visit if Android testing surfaces abuse.
  2. _Effort (STRICT):_ Text block > 4 words. Catches blank / one-line / OCR-failed uploads. Fails with `low_effort`.
  3. _Contact name (STRICT GATE — the anti-cheat rule):_ At least one trusted contact's `name` appears (case-insensitive substring) in the OCR text. Skipped if `contacts` is empty. Fails with `no_contact_name` and the toast names the gate explicitly ("We couldn't read a trusted contact's name. Make sure their name appears in the conversation — spelled exactly as you saved it in your messaging app.").
  4. _Time presence (SOFT signal — diagnostic only):_ A `HH:MM` 24-hour time-shape appears in the OCR text (the language-agnostic regex `/\b([01]\d|2[0-3]):[0-5]\d\b/`). Logged in dev. **Does NOT fail verification.** Rationale: OCR can't distinguish "today's 11:42" from "yesterday's 11:42" without a visible date marker, and the earlier multi-pattern recency check (`today` / `yesterday` / weekday names / `X mins ago` / 12-h clock) was English-only, which produced false negatives for users texting in other languages. We dropped both that multi-pattern check and the English-only `MESSAGING_SIGNALS` "context" check that used to require words like "send" / "message" / "delivered" in the OCR text. **Don't add them back.** The strict gate combo (age + name) plus the friction of going through the Manual Bypass after two fails is the actual anti-cheat layer.
  5. _Thematic relevance (SOFT GATE — drives rotation, never fails):_ When the matched contact has generated themes, we score each theme by keyword overlap and pick the strongest. **A zero-keyword-match does NOT fail verification** — it just means no theme's `used_at` advances for this connection. The user still gets through on the name match alone. This is intentional: keyword matching is too fragile to act as a hard gate, but it's good enough to drive rotation credit. Anyone reading `lib/ocr.ts` who doesn't know this might "fix" it back to strict — don't.
...
- **Edge Functions:** Five functions deployed via `supabase functions deploy <name>`:
  ...
  - `generate-themes` — writes 6 personalised prompts per contact via OpenAI (`gpt-4o-mini`, JSON mode). Request body: `{ contact_id, language? }`. Verifies the caller JWT, reads the contact row, calls OpenAI with a strict system prompt, deletes the contact's existing `used_at IS NULL` themes, inserts the new batch with 15-20 keywords each, returns them. Requires `OPENAI_API_KEY` secret (`supabase secrets set OPENAI_API_KEY=sk-...`).
    ...
    - **Output format:** themes are short action phrases in lowercase infinitive form with a literal `{name}` placeholder (e.g. `"ask {name} how the new job at the firm is going"`). Keywords are 5-8 **tightly relevant** tokens per theme (sanitiser caps at 8). The prompt explicitly bans both generic words ("happy", "good", "hope" — would credit the wrong prompt) and overly formal vocabulary ("congratulate", "achievement" — real texts use casual variants). Quality over quantity matters because the prompts are now visible to the user in the editor (§6E), so a match should mean "they wrote about THIS specific prompt", not "they wrote anything tangentially related". Earlier versions used 15-20 keywords for broader coverage; we tightened it once the soft thematic gate made coverage less important than precision.
...
- [ ] **PresenceShieldAction App ID + entitlements:** New extension target added in Phase post-8. In Apple Developer Portal create an App ID for `com.franciccio.presence.PresenceShieldAction`, enable the **App Groups** capability and add `group.com.franciccio.presence`. Then run `eas credentials --platform ios` to delete cached provisioning profiles so EAS generates fresh ones covering the new extension (otherwise the EAS build fails with a "no matching profile" error or, worse, builds without the extension and the "Open Presence" button silently does nothing in prod).
