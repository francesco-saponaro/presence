...
6. Insert into `block_challenges` (best-effort — local store is source of truth for the session; `resolveAllActiveOnServer` first clears any straggler so the partial unique index doesn't reject).
7. Idempotent by default: no-op if `activeChallenge` already set — pass `force: true` to reassign (used from `scheduleWarmupNotification`).

**Resolution (`resolveActiveChallenge()`):** called from `onConnectionVerified`. Returns `{ contactId, themeId, word }` from the snapshot, clears the local `activeChallenge`, marks the server row `resolved_at = now()`. The word flows into the proof row via `matched.challengeWord`.

**Assignment triggers (four — must stay in sync):**
1. `scheduleWarmupNotification` — force-assigns before baking the body so notification + Home show the same `(name, word)`. Fires at: app start, schedule save, `onConnectionVerified`, `regenerateThemes`, language change.
2. On-demand safety net in `app/(tabs)/index.tsx` — a `useEffect` that assigns when `isBlocked && !activeChallenge && contacts.length > 0`. Covers extension-fired blocks that didn't route through `scheduleWarmupNotification`.
3. Post-verify — cleared by `resolveActiveChallenge`, immediately re-populated because `onConnectionVerified` calls `scheduleWarmupNotification`.
4. Manual bypass path — `onConnectionVerified(true)` still calls `resolveActiveChallenge`, so the current challenge clears + a new one is assigned via the warm-up rebake.
...
**Notifications:**
- **Warm-up body** priority order: (1) `warmupBodyChallenge` when `activeChallenge` is set — "Shield up in 15 min. Text {name} something with the word \"{word}\" to unlock." (2) `warmupBodyTargeted` (legacy — no active challenge but a theme exists), (3) `warmupBodyNoTheme`, (4) `warmupBody` generic. `scheduleWarmupNotification` calls `assignChallengeIfNeeded({ force: true })` first via a **lazy `require`** so the `notifications → blockChallenge → contactsSync → notifications` import cycle is broken.
