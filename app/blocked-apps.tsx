...
export default function BlockedAppsScreen() {
  ...
  const EDIT_LOCK_WHILE_BLOCKED = true;
  ...
  async function openPicker() {
    ...
    try {
      ...
      if (result) {
        ...
        if (isBlocked) {
          if (count > 0) {
            ScreenTimeModule.applyShieldFromSelection(result.selection).catch(
              console.warn,
            );
          } else {
              ...
              {selectionCount > 0 && (
                <View className="mt-4 flex-row items-center gap-2 px-1">
                  ...
                  <Text className="font-sans-body text-sm text-brown-mid dark:text-greige">
                    {selectionCount} {selectionCount === 1 ? "app" : "apps"}{" "}
                    selected
                  </Text>
                </View>
              )}
